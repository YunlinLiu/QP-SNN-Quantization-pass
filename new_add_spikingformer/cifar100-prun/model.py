...
from quant_function import ReScaWConv2d, ReScaWConv1d
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.):
        ...
        self.mlp1_conv = ReScaWConv2d(in_features, hidden_features, kernel_size=1, stride=1)
        self.mlp1_bn = nn.BatchNorm2d(hidden_features)

        self.mlp2_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
        self.mlp2_conv = ReScaWConv2d(hidden_features, out_features, kernel_size=1, stride=1)
...
class SpikingSelfAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.q_conv = ReScaWConv1d(dim, dim, kernel_size=1, stride=1)
        self.q_bn = nn.BatchNorm1d(dim)

        self.q_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
        self.k_conv = ReScaWConv1d(dim, dim, kernel_size=1, stride=1)
        self.k_bn = nn.BatchNorm1d(dim)

        self.k_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
        self.v_conv = ReScaWConv1d(dim, dim, kernel_size=1, stride=1)
        ...
        self.proj_conv = ReScaWConv1d(dim, dim, kernel_size=1, stride=1)
...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.block0_conv = nn.Conv2d(in_channels, embed_dims // 8, kernel_size=3, stride=1, padding=1)
        self.block0_bn = nn.BatchNorm2d(embed_dims // 8)

        self.block1_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
        self.block1_conv = ReScaWConv2d(embed_dims // 8, embed_dims // 4, kernel_size=3, stride=1, padding=1)
        self.block1_bn = nn.BatchNorm2d(embed_dims // 4)

        self.block2_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
        self.block2_conv = ReScaWConv2d(embed_dims // 4, embed_dims // 2, kernel_size=3, stride=1, padding=1)
        ...
        self.block3_conv = ReScaWConv2d(embed_dims // 2, embed_dims // 1, kernel_size=3, stride=1, padding=1)
        ...
        self.block4_conv = ReScaWConv2d(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1)
