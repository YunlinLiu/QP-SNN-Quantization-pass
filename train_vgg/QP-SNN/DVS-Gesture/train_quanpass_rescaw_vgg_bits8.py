...
from models.pure_vggsnn import vggsnn


parser = argparse.ArgumentParser("quant train")
...
parser.add_argument(
    '--job_dir',
    type=str,
    default='/workspace/QP-SNN-Quantization-pass/output_vgg_quan/QP-SNN/DVS-Gesture/ReScaW_q8/',
...
parser.add_argument(
    ...
    type=str,
    help='dataset name',
    choices=['CIFAR10', 'CIFAR100', 'ImageNet', 'TinyImageNet', 'DVSCIFAR10', 'DVS128'])

parser.add_argument(
    '--time',
    type=int,
    default=16,
    help='number of frames for DVS128Gesture')
...
def main():
    ...
    model = vggsnn(compress_rate=[0.0]*8, num_classes=CLASSES)
    for p in model.parameters():
        p.requires_grad = True

    # apply ReScaW quantization pass (as in test)
    ReScaW_quan_pass_args = {
        "by": "regex_name",
        r"features\.convbn(?!0\b)\d+\.layer\.module": {
            "config": {
                "name": "rescaw",
                "num_bits": int(args.bit),
            }
        },
    }
    model, _ = quantize_module_transform_pass(model, ReScaW_quan_pass_args)
    print(model)
