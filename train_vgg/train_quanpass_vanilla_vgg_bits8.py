...
parser = argparse.ArgumentParser("TinyImageNet VGG-16 ReScaW Quantization")   # keep title unchanged (minimal edits)
...
parser.add_argument(
    '--job_dir',
    type=str,
    default='./output_vgg_quan/TinyImageNet/Vanilla_q8/',  # Output to Vanilla_q8 folder
...
parser.add_argument(
    '--epochs',
    type=int,
    default=300,  # Train phase: 300 epochs
    help='num of training epochs')

parser.add_argument(
    '--lr',
    type=float,
    default=0.1,  # Train phase: 0.1 initial learning rate
...
parser.add_argument(
    '--dataset',
    default='TinyImageNet',  # Default to TinyImageNet
    type=str,
    help='dataset name',
...
now = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S') # now = '2024-01-15-14-30-25' (example)
logger = common.get_logger(os.path.join(args.job_dir, 'logger'+now+'.log')) # Log file creation, file path: ./log_pass/logger2024-01-15-14:30:25.log
...
def main():
    ...
    # Train phase: Use SGD optimizer (as per paper Table 8)
    ...
    # Learning rate decay: Cosine (as per paper Table 8)
    ...
    if args.resume:
        checkpoint_dir = os.path.join(args.job_dir, 'checkpoint.pth.tar')   # Default: ./log_pass/checkpoint.pth.tar
        logger.info('loading checkpoint {} ..........'.format(checkpoint_dir))
        checkpoint = torch.load(checkpoint_dir)
        start_epoch = checkpoint['epoch'] + 1
        best_top1_acc = checkpoint['best_top1_acc']
        
        # Directly load state dictionary (keep GPU environment consistent)
