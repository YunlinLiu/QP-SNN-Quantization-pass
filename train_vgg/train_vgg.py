...
parser = argparse.ArgumentParser("cifar10 quant")   # Argument parser
...
logger = common.get_logger(os.path.join(args.job_dir, 'logger'+now+'.log')) # Log file creation, file path: ./log/logger2024-01-15-14:30:25.log
...
# if len(args.gpu)>1:
#     name_base='module.' # In PyTorch, when using nn.DataParallel for multi-GPU training, model parameter names automatically get module. prefix.
# else:
#     name_base=''
...
def main():
    ...
    if args.resume:
        checkpoint_dir = os.path.join(args.job_dir, 'checkpoint.pth.tar')   # Default: ./log/checkpoint.pth.tar
        ...
        # if len(args.gpu) > 1:   # Case 1: Currently using multi-GPU
        #     for k, v in tmp_ckpt.items():   
        #         new_state_dict['module.' + k.replace('module.', '')] = v    # Ensure all parameter names have module. prefix
        # else:
        #     for k, v in tmp_ckpt.items():
