...
parser.add_argument(
    ...
    type=str,
    default='0',
    help='Select gpu to use, e.g. "0,1,2,3"')
...
print_freq = (256*50)//args.batch_size #确定训练过程中多久打印一次日志
...
def main():
    ...
    # multi-GPU setup
    gpu_str = args.gpu.strip()
    if gpu_str:
        device_id = [int(x) for x in gpu_str.split(",") if x.strip() != ""]
    else:
        device_id = list(range(torch.cuda.device_count()))
    if len(device_id) > 1:
        logger.info(f"Using {len(device_id)} GPUs via DataParallel")
