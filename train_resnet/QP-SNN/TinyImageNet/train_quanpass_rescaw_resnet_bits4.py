...
import sys
# ensure repo root is first on sys.path to import top-level `utils`
...
parser = argparse.ArgumentParser("TinyImageNet ResNet-20")   # Argument parser
...
parser.add_argument(
    '--job_dir',
    type=str,
    default='/workspace/QP-SNN-Quantization-pass/output_resnet_quan/QP-SNN/TinyImageNet/ReScaW_q4/',  # Output to ReScaW_q4 folder
...
parser.add_argument(
    '--epochs',
    type=int,
    default=300,  # Train phase: 300 epochs (no pruning, no fine-tune needed)
...
logger = common.get_logger(os.path.join(args.job_dir, 'ResNet_TinyImageNet_logger'+now+'.log'))
