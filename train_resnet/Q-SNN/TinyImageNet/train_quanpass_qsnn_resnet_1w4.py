#!/usr/bin/env python3
# Train Q-SNN ResNet-20 on TinyImageNet using pass-based quantization (1w-4u)
...
parser = argparse.ArgumentParser("TinyImageNet ResNet-20 Q-SNN (1w-4u) Quant + Train")

parser.add_argument('--arch', type=str, default='resnet_20', help='architecture')
parser.add_argument('--job_dir', type=str, default='/workspace/QP-SNN-Quantization-pass/output_resnet_quan/Q-SNN/TinyImageNet/QSNN_1w4/', help='path for saving models')
...
logger = common.get_logger(os.path.join(args.job_dir, 'ResNet_TinyImageNet_QSNN_1w4_'+now+'.log'))
...
def create_quantized_model_via_pass(num_classes):
    logger.info('==> Creating ResNet-20 via Q-SNN pass (1w-4u)..')
    ...
    pass_args = {
        ...
        r"conv1_s\.layer\.module$": {
            "config": {"name": "apot", "num_bits": 4, "base_k": 2}
        },
        # hidden convs
        ...
        r"relu$": {"config": {"name": "lifspike_q", "num_bits": 4, "thresh": 1.0, "tau": 0.5}},
        r"layer\d+\.\d+\.relu[12]$": {"config": {"name": "lifspike_q", "num_bits": 4, "thresh": 1.0, "tau": 0.5}},
    }
