#!/usr/bin/env python3
# Train Q-SNN ResNet-20 on CIFAR-10 using pass-based quantization (1w-8u)
...
parser = argparse.ArgumentParser("CIFAR-10 ResNet-20 Q-SNN (1w-8u) Quant + Train")

parser.add_argument('--arch', type=str, default='resnet_20', help='architecture')
parser.add_argument('--job_dir', type=str, default='./output_resnet_quan/Q-SNN/CIFAR10/QSNN_1w8u/', help='path for saving models')
...
parser.add_argument('--dataset', default='CIFAR10', type=str, help='dataset name', choices=['CIFAR10'])
...
logger = common.get_logger(os.path.join(args.job_dir, 'ResNet_CIFAR10_QSNN_1w8u_'+now+'.log'))
...
def create_quantized_model_via_pass(num_classes):
    logger.info('==> Creating ResNet-20 via Q-SNN pass (1w-8u)..')
