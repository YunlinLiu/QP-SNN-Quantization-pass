...
class _WsdrMonitor:
    def __init__(self):
        super().__init__()
        self._handles = []
        self._f_list = []  # store per-layer firing rate tensors (with grad)
        self._registered = False

    def _is_lif_module(self, m: nn.Module) -> bool:
        name = m.__class__.__name__
        # cover spikingjelly MultiStepLIFNode and custom quantized variants
        return name in {
            'MultiStepLIFNode', 'MultiStepParametricLIFNode',
            'MultiStepLIFNodeQ', 'MultiStepLIFNodeQCuPy',
            'LIFSpike', 'LIFSpikeQ'
        }

    def register(self, model: nn.Module, skip_first: bool = True):
        if self._registered:
            return
        lif_modules = []
        for name, m in model.named_modules():
            if self._is_lif_module(m):
                lif_modules.append((name, m))

        # optionally skip the very first spiking layer as "input" layer
        if skip_first and len(lif_modules) > 0:
            lif_modules = lif_modules[1:]

        def hook_fn(_m, _inp, out):
            # out could be [T,B,...] or already with time dimension; compute mean firing rate
            try:
                f = out.float().mean()
            except Exception:
                # fall back to flatten then mean
                f = out.float().reshape(-1).mean()
            self._f_list.append(f)

        for _, m in lif_modules:
    ...
    def loss(self) -> torch.Tensor:
        if len(self._f_list) == 0:
            # return a zero tensor that lives on the right device to keep graph valid
            return torch.tensor(0.0, device=self._infer_device())
        losses = [(f - 0.5) * (f - 0.5) for f in self._f_list]
...
class vit_snn(nn.Module):
    def __init__(self,
        ...
        # WS-DR monitor (hooks registered later, after potential quantization pass)
        self._wsdr = _WsdrMonitor()
    ...
    # ---- WS-DR auxiliary APIs ----
    def register_wsdr_hooks(self, skip_first: bool = True):
        self._wsdr.register(self, skip_first=skip_first)

    def reset_wsdr_monitor(self):
        self._wsdr.reset()

    def get_wsdr_loss(self) -> torch.Tensor:
        return self._wsdr.loss()
