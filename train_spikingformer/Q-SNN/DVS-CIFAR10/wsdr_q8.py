...
import sys
project_root = Path(__file__).resolve().parents[3]  # 修正路径深度：3层
sys.path.append(str(project_root))
sys.path.append(str(project_root / 'Spikingformer' / 'cifar10-dvs'))
import model_wsdr as model, utils
...
def parse_args():
    ...
    parser.add_argument('--data-path', default='/workspace/QP-SNN-Quantization-pass/data/CIFAR10DVS', help='dataset')
    ...
    parser.add_argument('--output-dir', default='./output_spikingformer_quan/Q-SNN/DVS-CIFAR10/WSDR_q8', help='path where to save')
    ...
    parser.add_argument('--wsdr-lambda', type=float, default=1e-3, help='lambda for WS-DR spike regularization')
...
def split_to_train_test_set(train_ratio: float, origin_dataset: torch.utils.data.Dataset, num_classes: int, random_split: bool = False):
    ...
    train_idx, test_idx = [], []
    for i in range(num_classes):
        pos = int((len(label_idx[i]) * train_ratio + 0.9999))
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, print_freq, scaler=None, T_train=None, aug=None, trival_aug=None, mixup_fn=None, wsdr_lambda: float = 1e-3):
    model.train()
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # reset WS-DR monitor per-batch to compute this batch's firing rates
        if hasattr(model, 'reset_wsdr_monitor'):
            model.reset_wsdr_monitor()

        if scaler is not None:
            with amp.autocast():
                output = model(image)
                loss = criterion(output, target)
                if wsdr_lambda and hasattr(model, 'get_wsdr_loss'):
                    loss = loss + float(wsdr_lambda) * model.get_wsdr_loss()
        else:
            output = model(image)
            loss = criterion(output, target)
            if wsdr_lambda and hasattr(model, 'get_wsdr_loss'):
...
def main(args):
    ...
    # -- validate device to avoid invalid device ordinal --
    try:
    ...
    # ---- DVS variant (cifar10-dvs) Q-SNN pass args (bits=8 for voltage; apot 8-bit on patch/head) ----
    pass_args_dvs = {
        "by": "regex_name",
    ...
    # set all params trainable (as in test)
    for p in model.parameters():
        p.requires_grad = True

    model, _ = quantize_module_transform_pass(model, copy.deepcopy(pass_args_dvs))

    # register WS-DR hooks after quantization replacement
    if hasattr(model, 'register_wsdr_hooks'):
        model.register_wsdr_hooks(skip_first=True)
    ...
    for epoch in range(args.start_epoch, num_epochs):
        ...
        train_loss, train_acc1, train_acc5 = train_one_epoch(
            ...
            train_snn_aug, train_trivalaug, mixup_fn, wsdr_lambda=args.wsdr_lambda)
